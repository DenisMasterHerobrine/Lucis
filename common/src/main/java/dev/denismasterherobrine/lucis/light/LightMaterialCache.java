...
public final class LightMaterialCache {
    ...
    public LightMaterial lookup(BlockGetter level, BlockState state, BlockPos pos) {
        ...
        byte opacity = (byte) Math.max(0, Math.min(LucisConstants.MAX_LIGHT, state.getLightDampening()));
        ...
        return new LightMaterial(
                (byte) Math.max(0, Math.min(LucisConstants.MAX_LIGHT, state.getLightDampening())),
                emission,
                flags
        );
    }
